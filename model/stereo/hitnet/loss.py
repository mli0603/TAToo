...
def subpix_cost(cost: torch.Tensor, disp: torch.Tensor, maxdisp: int):
    ...
    sub_cost = (disp - disp_floor) * torch.gather(cost, 1, disp_floor.long() + 1) + (
            disp_floor + 1 - disp) * torch.gather(cost, 1, disp_floor.long())
    # pdb.set_trace()
    return sub_cost
...
class HITLoss(nn.Module):
    ...
    def __init__(self, max_disp=320, lambda_init=1, lambda_prop=1, lambda_slant=1, lambda_w=1, alpha=0.9, c=0.1,
                 loss_weight=1.0):
    ...
    def forward(self, init_cv_cost_pyramid, prop_disp_pyramid, dx_pyramid, dy_pyramid, w_pyramid, d_gt, meta):
        ...
        mask = compute_valid_mask(meta, gt_disp=d_gt)
        prop_loss_weights = [
            1 / 64,
            1 / 32,
            1 / 32,
        ...
        return self.loss_weight * torch.mean(total_loss_vec), losses
